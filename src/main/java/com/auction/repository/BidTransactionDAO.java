...
public class BidTransactionDAO {
    ...
    public boolean saveTransaction(int auctionId, BidTransaction tx) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            pstmt.setString(2, tx.getBidder().getId()); // Đảm bảo class Bidder của sếp có hàm getId() nhé
    ...
    // Thêm import này ở trên cùng file nếu chưa có
    // import java.sql.ResultSet;
    // import java.util.ArrayList;
    // import java.util.List;

    // HÀM MỚI: Lấy toàn bộ lịch sử đấu giá của một món hàng
    public void printTransactionHistory(int auctionId) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                String bidderId = rs.getString("bidder_id"); // Nếu sếp dùng String ID
                double amount = rs.getDouble("bid_amount");
                String time = rs.getString("timestamp");

                System.out.println("Thời gian: " + time + " | Đại gia ID: " + bidderId + " | Chốt giá: $" + amount);
            }
            System.out.println("===================================");
